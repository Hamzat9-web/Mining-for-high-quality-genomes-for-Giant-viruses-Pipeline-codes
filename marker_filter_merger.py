...
def group_sequences_by_query(filtered_faa_file, target_to_query, output_dir, base_name):
    ...
    query_counts = {}
    query_files = {}  # Track created files for merging later
...
def process_directory(dir_path, all_query_files):
    ...
    for markerout_file in markerout_files:
        ...
        query_counts, query_files = group_sequences_by_query(corresponding_faa, target_to_query, dir_path, species_name)
        ...
        if query_counts:
            ...
            # Add to master collection for merging
            for query_name, file_path in query_files.items():
                if query_name not in all_query_files:
                    all_query_files[query_name] = []
                all_query_files[query_name].append(file_path)
...
def merge_query_sequences(all_query_files, base_dir):
    """Merge all sequences of the same query type into master files."""
    
    print(f"\n" + "="*60)
    print("MERGING QUERY SEQUENCES ACROSS ALL DIRECTORIES")
    print("="*60)
    
    # Create master query_sequences directory
    master_query_dir = os.path.join(base_dir, "query_sequences")
    try:
        os.makedirs(master_query_dir, exist_ok=True)
        print(f"Created master directory: {master_query_dir}")
    except Exception as e:
        print(f"Error creating master directory {master_query_dir}: {e}")
        return False
    
    merged_counts = {}
    
    for query_name, file_list in all_query_files.items():
        if not file_list:
            continue
            
        print(f"\nMerging {query_name} sequences from {len(file_list)} directories...")
        
        # Create merged file
        merged_file = os.path.join(master_query_dir, f"merged_{query_name}.fasta")
        total_sequences = 0
            ...
            with open(merged_file, 'w') as merged_f:
                for file_path in file_list:
                    if os.path.exists(file_path):
                        print(f"  Adding sequences from: {os.path.basename(os.path.dirname(file_path))}")
                        ...
                        with open(file_path, 'r') as input_f:
                            ...
                            print(f"    Added {seq_count} sequences")
                    else:
                        print(f"  Warning: File not found: {file_path}")
            
            merged_counts[query_name] = total_sequences
            print(f"   Created {os.path.basename(merged_file)} with {total_sequences} total sequences")
            
        except Exception as e:
            print(f"   Error creating merged file for {query_name}: {e}")
    
    return merged_counts

def main():
    """Main function to process all directories and merge results."""
    ...
    processed_count = 0
    failed_count = 0
    all_query_files = defaultdict(list)  # Track all query files for merging
    
    # Process each subdirectory
    for subdir in sorted(subdirectories):
        subdir_path = os.path.join(base_dir, subdir)
        try:
            if process_directory(subdir_path, all_query_files):
                processed_count += 1
            else:
                failed_count += 1
        except Exception as e:
    ...
    # Merge sequences if we processed any directories successfully
    if processed_count > 0 and all_query_files:
        merged_counts = merge_query_sequences(all_query_files, base_dir)
        
        if merged_counts:
            print(f"\n" + "="*60)
            print("MERGING COMPLETE!")
            print(f"Created {len(merged_counts)} merged query files:")
            for query, count in sorted(merged_counts.items()):
                print(f"  merged_{query}.fasta: {count} sequences")
            print(f"\nMerged files located in: {os.path.join(base_dir, 'query_sequences')}")
        else:
            print(f"\n No sequences were merged")
    else:
        print(f"\nNo directories processed successfully - skipping merge step")
    
    print(f"\n" + "="*60)
    print("SCRIPT COMPLETE!")
    if processed_count > 0:
        print(f"Individual query files: Created in each directory's 'query_sequences' subdirectory")
        print(f"Merged query files: Created in master '{os.path.join(base_dir, 'query_sequences')}' directory")
        print(f"File naming patterns:")
        print(f"  Individual: [SpeciesName]_[QueryName].fasta")
        print(f"  Merged: merged_[QueryName].fasta")
